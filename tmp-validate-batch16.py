# batch16 검증: 스키마 + 금지 문자 + topic 표준 8분류 (C890 신설)
import json, io, re, glob

ref = json.load(io.open("_input-batch15-competency-shift-ai-era.json", encoding="utf-8"))
ref_keys = set(ref.keys())
banned = re.compile("[—–·…→←↑↓⇒" + "\U0001F300-\U0001FAFF☀-➿]")
TABS = {"AI 전략", "AX 교육", "업무 분석", "데이터 의사결정", "운영 시스템", "조직과 사람", "신사업 실행", "테이블 노트"}
ok = 0
files = sorted(glob.glob("_input-batch16-*.json"))
for f in files:
